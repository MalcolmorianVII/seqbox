...
def test_get_sample_source():
    # create sample source
    sample_source = SampleSource(sample_source_identifier='sample1')
    ...
    # add sample source to project
    project.sample_sources.append(sample_source)

    # Brittle code need a validator here to check if data already in db 
    # commit session
    ...
    assert get_sample_source(sample_info).sample_source_identifier == sample_source.sample_source_identifier
