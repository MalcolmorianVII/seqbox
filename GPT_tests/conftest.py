...
def test_client():
    ...
    with app.test_client() as testing_client:
        ...
        # drop the database after the test is completed
        db.drop_all()

@pytest.fixture(scope='session')
def test_db():
    """Session-wide test database."""
    engine = db.get_engine(app.config['SQLALCHEMY_DATABASE_URI'])
...
def mykrobe_res_info():
    ...
    all_fields = {'sample': 'sample_01', 'drug': 'isoniazid', 
                        'susceptibility': 'resistant', 'mykrobe_version': 'v1.0'}
    yield blank_sample,blank_drug,blank_susc,blank_mykrobe_version,all_fields
